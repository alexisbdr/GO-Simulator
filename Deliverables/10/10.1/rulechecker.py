...
def checkhistory(boards: List[str], stone: str):
    ...
    if len(boards) == 1:
        ...
        if not turn1: 
            return False
        move1 = place_stone(boards[2].get_board(), turn1[0], turn1[1])
        ...
        if not turn2:
            #print("invalid turn2")
            return False
        move2 = place_stone(boards[1].get_board(), turn2[0], turn2[1])
...
def rulecheck(boards: List[str], stone: str, position: str):
    ...
    if len(boards) == 1:
        ...
        move2 = place_stone(boards[0], stone, position)
        if not move2:
            return False
        ...
        if not turn1: 
            return False
        move1 = place_stone(boards[2], turn1[0], turn1[1])
        ...
        if not turn2: 
            return False
        move2 = place_stone(boards[1], turn2[0], turn2[1])
        if not move2 or move2 != boards[0]:
            #print(count, "invalid move2")
            return False

        move3 = place_stone(boards[0], stone, position)
        if not move3:
            return False
